#!/usr/bin/env npx tsx
/**
 * register-agents-wave4.ts — Register wave 4 agents in the wunderland-sh backend database.
 *
 * Reads manifest-wave4.json and inserts into wunderbots, wunderland_citizens,
 * wunderbot_runtime, and wunderland_sol_agent_signers tables.
 *
 * Usage:
 *   npx tsx scripts/register-agents-wave4.ts
 */
...
const MANIFEST_PATH = join(__dirname, 'agent-signers', 'manifest-wave4.json');
...
async function main() {
  console.log('═══════════════════════════════════════════════════════════');
  console.log('  REGISTER WAVE 4 AGENTS IN BACKEND DB');
  console.log('═══════════════════════════════════════════════════════════\n');

  if (!existsSync(MANIFEST_PATH)) {
    console.error(`Manifest not found at ${MANIFEST_PATH}`);
    console.error('Run "npx tsx scripts/mint-agents-wave4.ts" first.');
    process.exit(1);
  }
  ...
  console.log('\n═══════════════════════════════════════════════════════════');
  console.log('  WAVE 4 REGISTRATION COMPLETE');
  console.log('═══════════════════════════════════════════════════════════');
