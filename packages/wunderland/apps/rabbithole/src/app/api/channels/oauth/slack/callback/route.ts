...
/**
 * Slack redirects users' browsers here after OAuth authorization.
...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(
      new URL(`/app/dashboard?oauth_error=${encodeURIComponent(error)}`, origin),
    );
  }
  ...
  try {
    const res = await fetch(`${API_BASE}/wunderland/channels/oauth/slack/callback`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.redirect(
      new URL(`/app/dashboard/${seedId}/channels?connected=slack`, origin),
    );
  } catch {
    return NextResponse.redirect(
