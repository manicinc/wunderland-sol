/**
 * Contribution modal for creating new Codex strands
 * (local copy to ensure props match QuarryViewer usage).
 */
...
const getSlug = () => form.customSlug || form.title.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')

const generateMarkdown = (slug: string) => {
  const id = generateId()
...
`
  ...
  const handleSubmit = async () => {
    ...
    try {
      const slug = getSlug()
      const markdown = generateMarkdown(slug)
...
*Submitted via Quarry Codex contribution form*
*${form.enableAI ? 'AI analysis will run automatically after PR is created' : 'Static NLP analysis only'}*`

      if (!githubPAT) {
        const githubUrl = `https://github.com/${process.env.NEXT_PUBLIC_CODEX_REPO_OWNER || 'framersai'}/${process.env.NEXT_PUBLIC_CODEX_REPO_NAME || 'codex'}/new/${process.env.NEXT_PUBLIC_CODEX_REPO_BRANCH || 'main'}?filename=${encodeURIComponent(
          filepath,
        )}&value=${encodeURIComponent(markdown)}`
        window.open(githubUrl, '_blank')
        setStep('success')
      } else {
        const response = await fetch('https://api.github.com/repos/framersai/quarry/forks', {
          method: 'POST',
          headers: {
        ...
        if (!fileResponse.ok) throw new Error('Failed to create file in fork')

        const prResponse = await fetch('https://api.github.com/repos/framersai/quarry/pulls', {
          method: 'POST',
          headers: {
            Authorization: `token ${githubPAT}`,
            Accept: 'application/vnd.github.v3+json',
                      ...
                      <label className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-semibold text-gray-900 dark:text-gray-100">Enable AI Enhancement</span>
                          </div>
                          <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                            AI will analyze content, suggest better tags/categories, and provide quality scoring. Cost: ~$0.01-0.20 per submission.
                          </p>
                        </div>
                      </label>
                    </div>
                    ...
                    <details className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700">
                      ...
                      <div className="mt-3">
                        ...
                        <p className="text-xs text-gray-500 mt-2">
                          Provide a PAT to automatically create PR via API. Without it, we'll open the GitHub editor for you.
                        </p>
                      </div>
                    </details>
                      ...
                      <div className="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-800">
                        <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">
                          <strong>File path:</strong> <code className="px-2 py-0.5 bg-gray-200 dark:bg-gray-800 rounded font-mono text-xs">
                            weaves/{form.weave}/{form.loom}/{getSlug()}.md
                          </code>
                        </p>
