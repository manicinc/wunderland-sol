...
import type { Bookmark, HistoryEntry, UserPreferences } from '@/lib/localStorage'
import {
  getBookmarks,
  addBookmark,
  removeBookmark,
...
export function useBookmarks(): UseBookmarksResult {
  ...
  const recordView = useCallback((path: string, title: string) => {
    // Respect user preference for history tracking (local-only)
    const prefs: UserPreferences = getPreferences()
    if (prefs.historyTrackingEnabled === false) {
      return
    }
