/**
 * SQL-backed cache for Quarry Codex client-side data.
 *
 * @remarks
...
async function getAdapter(): Promise<StorageAdapter | null> {
  ...
  if (!adapterPromise) {
    adapterPromise = createDatabaseFn({
      // Use IndexedDB adapter with sql.js WASM for browser persistence
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      priority: ['indexeddb', 'memory'] as any,
      type: typeof window === 'undefined' ? 'memory' : undefined,
      // Configure sql.js WASM location for browser deployment
      // Use window.location.origin to ensure absolute path works regardless of current route
      indexedDb: {
        dbName: 'fabric_codex',
        storeName: 'codex_strands',
        autoSave: true,
        saveIntervalMs: 3000,
        sqlJsConfig: {
