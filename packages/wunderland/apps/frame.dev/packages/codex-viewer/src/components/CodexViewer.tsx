...
import type { GitHubFile, SidebarMode, QuarryQuarryViewerProps } from '../lib/types'
import { parseWikiMetadata, shouldIgnorePath, isMarkdownFile } from '../lib/utils'
import { API_ENDPOINTS, REPO_CONFIG } from '../lib/constants'
...
import { useQuarryPath } from '../hooks/useQuarryPath'
...
export default function QuarryQuarryViewer({
    ...
    const initializeGroups = async () => {
      if (knowledgeTree.length > 0) {
        try {
          ...
          console.error('[QuarryViewer] Failed to generate default groups:', err)
        }
      }
    }
  ...
  useCodexHotkeys({
    ...
    onGoHome: () => router.push(resolvePath('/quarry')),
  ...
  const content = (
    <div className="quarry-codex-viewer flex-1 flex overflow-hidden">
            ...
            <>
              <HighlightSelector
                ...
                onHighlightCreated={(id) => {
                  reloadHighlights()
                  console.log('[QuarryViewer] Highlight created:', id)
                }}
              />
              <BlockHighlighter
                filePath={selectedFile.path}
                ...
                onHighlightCreated={(id) => {
                  reloadHighlights()
                  console.log('[QuarryViewer] Block highlight created:', id)
                }}
              />
            </>
          )}
      ...
      <MobileBottomNav
        ...
        onHome={() => {
          router.push(resolvePath('/quarry'))
          setSidebarOpen(false)
        }}
        onSearch={() => {
          const input = document.getElementById('codex-search-input') as HTMLInputElement | null
