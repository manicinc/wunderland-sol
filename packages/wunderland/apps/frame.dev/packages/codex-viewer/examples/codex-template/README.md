...
- Each markdown file under `weaves/**` automatically compiles to `/quarry/<slug...>` using App Router, SSR, and 1-hour ISR (`revalidate = 3600`).
...
5. Deploy. The `/` route includes the embedded viewer; `/quarry/**` serves static HTML for crawlers and deep-links.
