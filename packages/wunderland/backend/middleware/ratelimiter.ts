...
class InMemoryRateLimitStore implements RateLimitStore {
  ...
  private cleanup(): void {
    ...
    if (deletedCount > 0) {
      this.logger.info(
        `Cleaned up ${deletedCount} expired entries. Store size: ${this.store.size}`
      );
    }
  }
...
class RedisRateLimitStore implements RateLimitStore {
  ...
  async initialize(): Promise<void> {
    ...
    if (this.redis.status !== 'ready' && this.redis.status !== 'connect') {
      try {
        ...
        this.logger.error(
          `Failed to connect to Redis on initialize: ${getErrorMessage(error)}. Further operations may fail.`
        );
  ...
  async increment(baseKey: string, _windowSeconds: number): Promise<number> {
    ...
    try {
      ...
      this.logger.error(
        `Increment failed for key ${key}: ${getErrorMessage(error)}. This may indicate Redis is down.`
      );
...
export class RateLimiter {
  ...
  private readonly userMinuteCounters: Map<string, { count: number; expiresAt: number }> =
    new Map();
  ...
  constructor() {
    ...
    this.tierRpmConfigs = {
      ...
      unlimited: {
        maxRequestsPerMinute: parseInt(process.env.RATE_LIMIT_UNLIMITED_RPM || '120', 10),
      },
  ...
  public async initialize(): Promise<void> {
    if (process.env.REDIS_URL) {
      ...
      this.redisClient.on('error', (error: Error) => {
        ...
        if (this.isRedisAvailable) {
          ...
          this.redisLogger.error(
            `Global Redis client error: ${getErrorMessage(error)}. May fallback if persistent.`
          );
      ...
      this.redisClient.on('close', () => {
        if (this.isRedisAvailable) {
          // Log only if we thought it was connected
      ...
      try {
        ...
        this.logger.warn(
          `Failed to connect to Redis or ping: ${getErrorMessage(error)}. Falling back to InMemoryRateLimitStore.`
        );
        this.isRedisAvailable = false;
    ...
    setInterval(() => {
      ...
      this.userMinuteCounters.forEach((val, key) => {
        if (val.expiresAt < now) {
          this.userMinuteCounters.delete(key);
          cleaned++;
        }
      });
    ...
    this.logger.info(
      `Initialized. Public daily IP limit: ${this.publicIpRateLimitPerDay}. Tier RPM: public=${this.tierRpmConfigs.public.maxRequestsPerMinute}, metered=${this.tierRpmConfigs.metered.maxRequestsPerMinute}, unlimited=${this.tierRpmConfigs.unlimited.maxRequestsPerMinute}, global=${this.tierRpmConfigs.global.maxRequestsPerMinute}. Store: ${this.isRedisAvailable ? 'Redis' : 'In-Memory'}`
    );
  }
  ...
  public middleware() {
    return async (req: RequestWithUser, res: Response, next: NextFunction) => {
      if (!this.store) {
        this.logger.error(
          'CRITICAL: Store not initialized. Rate limiting disabled. Call initialize().'
        );
        return next();
      }
      if (req.user && req.user.id) {
        const tier =
          (req.user as any).tier || (req.user as any).mode === 'global' ? 'global' : 'metered';
        ...
        if (counter.count > rpmConfig.maxRequestsPerMinute) {
          this.logger.warn(
            `User ${userId} (${tier}) exceeded RPM limit. Count: ${counter.count}/${rpmConfig.maxRequestsPerMinute}`
          );
      ...
      if (ip === 'unknown' && process.env.NODE_ENV !== 'development') {
        return res
          .status(403)
          .json({ error: 'FORBIDDEN', message: 'Could not determine your IP address.' });
      }
      if (ip === 'unknown' && process.env.NODE_ENV === 'development') {
        return next();
      }
      ...
      try {
        ...
        if (currentCount > config.maxRequests) {
          this.logger.warn(
            `IP ${ip} exceeded public daily limit. Count: ${currentCount}/${config.maxRequests}`
          );
          return res.status(429).json({
            error: 'RATE_LIMIT_EXCEEDED',
        ...
        this.logger.warn(
          `Error during rate limiting store operation (IP: ${ip}): ${getErrorMessage(error)}. Failing open for this request.`
        );
  ...
  public async getPublicUsage(
    ...
    try {
      ...
      this.logger.warn(
        `Error fetching public usage from store for IP ${ip}: ${getErrorMessage(error)}. Assuming 0 usage.`
      );
