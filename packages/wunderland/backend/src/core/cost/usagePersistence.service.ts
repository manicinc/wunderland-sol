...
export const usagePersistenceService = {
  ...
  async pruneOldRecords(retainDays: number = 90): Promise<number> {
    try {
      const db = await getAppDatabase();
      const cutoffDate = new Date(Date.now() - retainDays * 86_400_000).toISOString().slice(0, 10);
      const result = await db.run('DELETE FROM usage_daily_ledger WHERE date_key < ?', [
        cutoffDate,
      ]);
