...
import {
  ...
  type CreditContext,
  type CreditSnapshot,
} from './creditAllocation.service.js';
    ...
    if (!snapshot.llm.isUnlimited && (snapshot.llm.remainingUsd ?? 0) <= 0.0005) {
      return {
        ...
        error: {
          ...
          message:
            'Your daily AI credit allowance has been used up. Credits reset at midnight UTC. Upgrade for higher limits.',
          details: {
    ...
    if (!snapshot.speech.isUnlimited && (snapshot.speech.remainingUsd ?? 0) <= 0.0005) {
      return {
        ...
        error: {
          ...
          message:
            'Your daily speech credit allowance has been used up. Credits reset at midnight UTC.',
          details: {
            allocationKey: snapshot.allocationKey,
